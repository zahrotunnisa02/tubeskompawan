  ...
  fetch('get_cafes.php') // Mengambil data kafe dari API PHP
    ...
    .then(cafes => {
      ...
      nearbyCafes.forEach(cafe => {
        // Debug: Cek struktur data cafe
        console.log(cafe);  // Pastikan image_url ada di data

        const marker = L.marker([cafe.latitude, cafe.longitude])
        .addTo(map)
        .bindPopup(`
          <b>${cafe.name}</b><br>
          Deskripsi: ${cafe.description ? cafe.description.substring(0, 100) + '...' : 'Deskripsi tidak tersedia.'}<br>
          Jarak: ${cafe.distance.toFixed(2)} km<br>
          <img src="../${cafe.image_url}" alt="${cafe.name}" style="width: 100px; height: auto; margin-top: 10px;" /><br>
          <button onclick="window.location.href='detail_cafe.php?id=${cafe.id}'" style="margin-top: 10px;">Detail Cafe</button>
        `);
